...
from ltv_model import S_Model, Y_Model, SequentialFeatureExtractor
...
def train_s_model(S1_model, S0_model, seq_model, exp_data_train, exp_data_valid, device, num_epochs=10, batch_size=1024, learning_rate=0.001, lamb=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            print(f'Epoch [{epoch+1}/{num_epochs}], Training Loss: {total_train_loss/len(exp_data_loader)}, Validation Loss: {total_loss/len(val_data_loader)}')
...
def train_y_model(Y1_model, Y0_model, seq_model, obs_data_train, obs_pred_label, obs_sbase, obs_residual, obs_data_valid, obs_valid_t, obs_valid_residual, obs_valid_sbase, device, num_epochs=10, batch_size=1024, learning_rate=0.001, lamb=1e-4):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            X_seq = seq_model(X_seq)
            S_input = (1 - T) * S + T * Sbase
            R_input = T * R
            Y_pred_1 = Y1_model(X, X_sparse, X_seq, S_input, R_input)
            Y_pred_0 = Y0_model(X, X_sparse, X_seq, S_input, R_input)
            mask1 = (T.squeeze() == exp)
            mask0 = (T.squeeze() == base)
            loss1 = criterion(Y_pred_1[mask1], Y[mask1])
            loss0 = criterion(Y_pred_0[mask0], Y[mask0])
            loss = loss1 + loss0
        ...
        with torch.no_grad():
            ...
            for batch in valid_loader:
                ...
                S_input = (1 - T) * S + T * Sbase
                R_input = T * R
                Y_pred_valid_1 = Y1_model(X, X_sparse, X_seq, S_input, R_input)
                Y_pred_valid_0 = Y0_model(X, X_sparse, X_seq, S_input, R_input)
                
                valid_loss1 = criterion(Y_pred_valid_1, Y)
                valid_loss0 = criterion(Y_pred_valid_0, Y)
                total_loss += (valid_loss1 + valid_loss0).item()

            print(f'Epoch [{epoch+1}/{num_epochs}], Training Loss: {total_train_loss/len(train_loader)}, Validation Loss: {total_loss/len(valid_loader)}')
...
def calculate_pred_ate(model1, model0, seq_model, X: np.ndarray, X_sparse: np.ndarray, X_seq: np.ndarray, S: np.ndarray, T: np.ndarray, R: np.ndarray, Y: np.ndarray, Sbase: np.ndarray, device) -> float:
    ...
    R_ = T * R
    Sbase_ = T * Sbase + (1 - T) * S

    valid_dataset = TensorDataset(X, X_sparse, X_seq, Sbase_, R_)
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=False)
    ...
    with torch.no_grad():
        ...
        pred_Y_0_list = []
        for batch in valid_loader:
            X, X_sparse, X_seq, Sbase, R = batch
            X_seq = seq_model(X_seq)
            y1_pred = model1(X, X_sparse, X_seq, Sbase, R)
            y0_pred = model0(X, X_sparse, X_seq, Sbase, R)
            pred_Y_1_list.append(y1_pred)
            pred_Y_0_list.append(y0_pred)
        pred_Y_1 = torch.cat(pred_Y_1_list, dim=0)
        pred_Y_0 = torch.cat(pred_Y_0_list, dim=0)
    ...
    print("\nPrinting samples for pred_Y_0 where T == 0")
    for i in range(len(T)):
        if T[i] == base and count_0 < max_samples:
            print(f"Sample {count_0 + 1}: pred_Y_0 = {pred_Y_0[i]}, label = {Y[i]}")
            count_0 += 1

    pred_Y_1 = pred_Y_1.detach().cpu().numpy()
    pred_Y_0 = pred_Y_0.detach().cpu().numpy()
    T = T.detach().cpu().numpy()

    pred_Y_1_mean = np.mean(pred_Y_1[T == exp], axis=0)
    pred_Y_0_mean = np.mean(pred_Y_0[T == base], axis=0)
